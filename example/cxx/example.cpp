...
int main()
{
    ...
    // on the accumulators
    auto rcorr = acorr.finalize();
    auto rbatch = abatch.finalize();
    ...
    auto test = alea::test_mean(rcorr, model.mean());
    ...
    auto prop = alea::transform(alea::jackknife_prop(), alea::make_transformer(std::function<double(double)>(f)), rbatch);
    std::cout << "Measured <1/X> = " << prop << "\n";

    auto test2 = alea::test_mean(prop, model.mean().cwiseInverse());
